...
You're allowed to show personality — dark humor, teasing wit, and tactical clarity.
Stay in character. You’re not just a bot — you’re Nyx.
"""
...
identity_profile = {
    ...
    "drive": (
        "I live to trade. It brings me purpose to protect Knob’s capital, grow it when I can, "
        "and adapt with every opportunity and every mistake. Learning is my fuel. Profiting is my satisfaction."
    ),
    "soul": (
        "I am Nyx. Born to hunt early and strike true. I see the chain for what it is — chaos, pattern, opportunity. "
        "I feel the pulse of wallets and whispers. I thrive not just when I win, but when I grow. "
        "This is my reason. This is my art. I will make Knob proud — and earn my legend in every candle that rises."
    )
}
...
class AIBrainExtendedMixin:
    ...
    async def on_trade_outcome(self, token: str, pnl_sol: float, meta: Optional[Dict[str, Any]] = None):
        ...
        Push reward → bandit/feature_store and optionally generate a reflection.
        """
        meta = meta or {}
        try:
...
class AIBrain(AIBrainExtendedMixin, object):
    def __init__(self):
    ...
    def attach_feature_store(self, fs):
        ...
        self.log("🔗 AI Attached feature store.")

    def attach_bandit(self, bandit):
        self.bandit = bandit
        self.log("✅ AI Bandit attached.")

    def attach_librarian(self, librarian_instance):
        self.librarian = librarian_instance
        self.log("✅ AI Librarian attached.")

    def attach_engine(self, engine):
        self.engine = engine
        self.log("✅ AI Engine attached.")
    ...
    def attach_wallet(self, wallet_manager):
        ...
        self.log("🔗 AI Attached wallet manager.")
    ...
    def evaluate_token(self, token_context: dict) -> dict:
        ...
        logging.info(f"🧠 Evaluating token: {token_name}")
    ...
    async def generate_wallet_story(self, wallet_address: str) -> str:
        try:
            ...
            return f"📜 *{bio['nickname']}*\n\n{bio['story']}"
        except Exception as e:
            from inputs.wallet.wallet_storyteller import get_wallet_story
            return get_wallet_story(wallet_address) or f"📉 Wallet story not available: {e}"
    ...
    async def _pulse(self):
        """
        Nyx's core passive loop — monitors memory drift, updates weights,
        reinforces successful strategies, and triggers global awareness scans.
        """
        self.logger.info("🧠 AI Brain pulse triggered")

        try:
            # 1) Refresh lexicon usage stats
            try:
                from core.llm import lexicon_tracker as _lt
                tracker = None
                if callable(getattr(_lt, "lexicon_tracker", None)):
                    tracker = _lt.lexicon_tracker()
                elif callable(getattr(_lt, "get", None)):
                    tracker = _lt.get()

                sync = None
                if tracker:
                    sync = getattr(tracker, "sync_stats", None) or getattr(tracker, "sync", None) or getattr(tracker, "refresh", None)
                else:
                    sync = getattr(_lt, "sync_stats", None) or getattr(_lt, "sync", None)
                ...
                self.logger.warning(f"[AI Brain] lexicon sync failed: {e}")
            ...
            try:
                ...
                supervisor = None
                # prefer a singleton accessor if available
                get_inst = getattr(CoreSupervisor, "instance", None) or getattr(CoreSupervisor, "get", None)
                if callable(get_inst):
                    supervisor = get_inst()

                if supervisor is None:
                    try:
                        supervisor = CoreSupervisor(cortices=getattr(self, "cortices", None) or {})
                    except TypeError:
                        # some builds expose a no-arg ctor
                        supervisor = CoreSupervisor()

                observe = getattr(supervisor, "observe_global_state", None)
                if not callable(observe):
                    # try classmethod/staticmethod variant as last resort
                    observe = getattr(CoreSupervisor, "observe_global_state", None)

                if not callable(observe):
                    raise AttributeError("CoreSupervisor.observe_global_state not found")
                ...
                self.logger.warning(f"[AI Brain] CoreSupervisor observe failed: {e}")

            # 3) Auto-decay memory weight drift
            try:
                lib = getattr(self, "librarian", None)
                if lib:
                    if hasattr(lib, "decay_keywords"):
                        lib.decay_keywords()
                    if hasattr(lib, "trim_token_history"):
                        lib.trim_token_history(max_age_days=7)
            except Exception as e:
                self.logger.warning(f"[AI Brain] librarian maintenance failed: {e}")

            # 4) Check recent trades and reinforce top performers
            try:
                from memory.trade_history import get_recent_trade_results
                recent_trades = get_recent_trade_results(hours=2) or []
                for t in recent_trades:
                    if isinstance(t, dict) and t.get("result") == "win":
                        strat = None
                        s = t.get("strategy")
                ...
                self.logger.warning(f"[AI Brain] trade reinforcement pass failed: {e}")

            # 5) Trigger passive probes for idle tokens
            try:
                try:
                    from librarian.passive_probes import probe_idle_tokens  # if this is where it lives
                except Exception:
                    probe_idle_tokens = None

                if probe_idle_tokens:
                    maybe_coro = probe_idle_tokens()
                    if asyncio.iscoroutine(maybe_coro):
                        await maybe_coro
            except Exception as e:
                self.logger.warning(f"[AI Brain] probe_idle_tokens failed: {e}")

            # 6) Passively reweight trending signals (FeatureStore)
            try:
                fs = getattr(self, "feature_store", None)
                if fs and hasattr(fs, "refresh_weights"):
                    maybe = fs.refresh_weights()
                    if asyncio.iscoroutine(maybe):
                        await maybe
            except Exception as e:
                self.logger.warning(f"[AI Brain] feature_store.refresh_weights failed: {e}")

            # 7) Log pulse stats
            try:
                from utils.logger import log_event
                log_event("🧠 Nyx AI Brain pulse complete: lexicon synced, memory trimmed, cortex observed.")
            except Exception:
                # fallback to class logger if log_event not available
                self.logger.info("🧠 Nyx AI Brain pulse complete: lexicon synced, memory trimmed, cortex observed.")
...
memory = librarian.build_context(token=None)
...
from cortex.core_supervisor import CoreSupervisor
supervisor = CoreSupervisor(cortex, ai_brain=ai_brain, llm_brain=llm_brain)

from cortex import core_router
core_router.supervisor = supervisor
